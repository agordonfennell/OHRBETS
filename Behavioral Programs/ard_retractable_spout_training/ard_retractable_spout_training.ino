...
// session parameters *************************************************************************************************************
 ...
 // access & solenoid --------
  ...
  static byte sol_duration = 12;          // duraiton solenoid is open during delivery
  static byte num_sol = 5;                // number of solenoid opening access period
  static int inter_sol_time = 180 - sol_duration;        // interval between each solenoid opening (measured from end of previous until start of next) should be 180 total
 
 // tone cue -----------------
  static unsigned long tone_freq = 5000;  // tone played during access period / laser stim period (Hz)
  static int tone_duration = 5000; // tone duration (starts with begining of access time)
  static int tm_delay_tone_access = 5000; // delay between tone onset and start of access period
...
// arduino pins ----------------------------------------------------------------------------
 ...
 // outputs ---------------------------
  ...
  static byte pinTone_ttl = 53; 
  static byte pinLED = 10;
...
// servo break variables / parameters ------------------------------------------------------
  ...
  static byte servo_break_engaged_deg = 30;
...
// session initialization (runs once at start) -----------------------------------------------------------------
  if(first_loop){
   ...
   // set time for first delivery
    ts_tone_start = ts + random(min_iti, max_iti);           // calculate first trial start
...
// licking-----------------------------------------------------------------------------------------------------
 ...
 // start access period------------
  if(ts >= ts_tone_start && ts_tone_start != 0){
    ...
    digitalWrite(pinTone_ttl, HIGH);
    digitalWrite(pinLED, HIGH);
    ts_tone_ttl_off = ts + tone_duration;
    ts_access_start = ts + tm_delay_tone_access;
